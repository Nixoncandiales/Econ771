...
pacman::p_load(tidyverse,vroom, here, sqldf)
...
1.  Provide and discuss a table of simple summary statistics showing the
    ...
    and patients at the physician level. You can do this using the
    average Medicare allowed amt \* bene_day_srvc_cnt (or Medicare
    allowed amt \* line_srvc_cnt) for spending, bene_day_srvc_cnt or the
    line_srvc_cnt for claims, and bene_unique_cnt for patients. The
    patient counts will include some overlap since the data are by
    service, but that's OK for our purposes.
...
3.  Estimate the relationship between integration on total physician
    claims using OLS, with the following specification:
...
5.  where $INT_{it}$ is defined in Equation \@ref(eq:int), $x_{it}$
    captures time-varying physician characteristics, and $\gamma_{i}$
    and $\gamma_{t}$ denote physician and time fixed effects.

6.  How much should we be "worried" about endogeneity here? Extending
    ...
    (\#eq:oster)
    \end{equation} where $x_{1}$ captures our observable covariates;
    $\delta$ denotes the treatment effect of interest;
    ...
    relative to unobserved variables
...
7.  Construct the total change in Medicare payments achievable for an
...
8.  Assess the "need" for IV by implementing a Durbin-Wu-Hausman test
...
9.  Now let's pay attention to potential issues of weak instruments. As
...
10. Following the Borusyak and Hull (2021) working paper (BH), we can
...
11. Discuss your findings and compare estimates from different
    estimators.

12. Reflect on this assignment. What did you find most challenging? What
