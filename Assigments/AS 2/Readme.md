Assigment 2 - Empirical Exercise
================
...
``` r
pacman::p_load(tidyverse,vroom, here, sqldf, ggthemes, fixest, modelsummary)
setwd("~/Documents/GitHub/Econ771/Assigments/AS 2")
here::i_am("Main.Rmd")
```

    ## here() starts at /Users/nix/Documents/GitHub/Econ771/Assigments/AS 2

``` r
# # All Data
# # source(here("Code", "Merge.R")) # Just need to run it once for ever !!!!!
# if (!exists("dat.MDPPAS")) dat.MDPPAS <- vroom(here("Output", "dat.MDPPAS.csv")) #Read the merged data MDPPAS
# if (!exists("dat.PUF")) dat.PUF <- vroom(here("Output", "dat.PUF.csv")) #Read the merged data PUF
# if (!exists("dat.PFS")) dat.PFS <- vroom(here("Output", "dat.PFS.csv")) #Read the merged data PFS
# 
# # Function to load a 1% sample of the Data Set... to speed coding.
# source(here("Code", "Rdsample.R"))
# # Random Sample.
...
```
    ...
    ##   <chr>      <dbl>    <dbl> <dbl>    <dbl>
    ## 1 Spending 10306.    43516.  0.11 6754003.
    ## 2 Claims     204.     2211. 11     466800 
    ## 3 Patients    74.6     137. 11      23941
...
2.  Form a proxy for integration using the ratio: where $HOPD_{it}$
    ...
    number of claims billed to an ambulatory surgery center. As
    reflected in Equation @ref(eq:int), you can assume that any
    physician with at least 75% of claims billed in an outpatient
    setting is integrated with a hospital. Using this 75% threshold,
    plot the mean of total physician-level claims for integrated versus
    non-integrated physicians over time.
...
![](Main_files/figure-gfm/q2-1.png)<!-- -->
...
3.  Estimate the relationship between integration on total physician
    ...
    remaining questions. Feel free to experiment with different
    covariates in $x_{it}$ or simply omit that term and only include the
    fixed effects.
...
```
    ...
    ## NOTE: 9,750 observations removed because of NA values (RHS: 9,750).
...
|                              | Model 1  |
|:-----------------------------|:--------:|
| average_submitted_chrg_amt   |  0.000   |
|                              | (0.000)  |
| average_medicare_payment_amt |  0.000   |
|                              | (0.000)  |
| int                          |  -0.011  |
|                              | (0.019)  |
| Num.Obs.                     |  368063  |
...
4.  How much should we be “worried” about endogeneity here? Extending
    ...
    $x_{1}$ captures our observable covariates (or fixed effects in our
    case); $\delta$ denotes the treatment effect of interest;
    $\hat{\delta}_{D,x_{1}}$ denotes the coefficient on $D$ from a
    regression of $y$ on $D$ and $x_{1}$; $R_{D,x_{1}}^{2}$ denotes the
    $R^{2}$ from that regression; $\hat{\delta}_{D}$ denotes the
    coefficient on $D$ from a regression of $y$ on $D$ only; $R_{D}^{2}$
    reflects the $R^{2}$ from that regression; $R_{max}^{2}$ denotes an
    unobserved “maximum” $R^{2}$ from a regression of $y$ on $D$,
    observed covariates $x_{1}$, and some unobserved covariates $x_{2}$;
    and $\rho$ denotes the degree of selection on observed variables
    relative to unobserved variables. One approach that Oster suggests
    is to consider a range of $R^{2}_{max}$ and $\rho$ to bound the
    estimated treatment effect, where the bounds are given by
