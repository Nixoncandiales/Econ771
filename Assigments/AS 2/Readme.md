...
``` r
pacman::p_load(tidyverse,vroom, here)
...
``` r
list.files(path = paste0("/Users/nix/Documents/GitHub/Econ771/Assigments/AS 2/", "Output"),
             recursive = TRUE,
             pattern = "\\.csv$",
...
df.list <- lapply(list_files, vroom)
```

    ## Rows: 5349907 Columns: 61

    ## ── Column specification ────────────────────────────────────────────────────────
    ## Delimiter: ","
    ## chr (10): name_last, name_first, name_middle, sex, birth_dt, spec_prim_1, sp...
    ## dbl (35): npi, spec_broad, spec_prim_2, pos_office, pos_inpat, pos_opd, pos_...
    ## lgl (16): phy_zip5, claim_count5, phy_zip6, claim_count6, phy_zip7, claim_co...
    ## 
    ## ℹ Use `spec()` to retrieve the full column specification for this data.
    ## ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
    ## Rows: 49283 Columns: 28
    ## ── Column specification ────────────────────────────────────────────────────────
    ## Delimiter: ","
    ## chr  (5): hcpcs, proc_stat, si, apc, betos_cat
    ## dbl (23): year, price_fac_trans, price_nonfac_trans, price_fac_full, price_n...
    ## 
    ## ℹ Use `spec()` to retrieve the full column specification for this data.
    ## ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
    ## Rows: 40151987 Columns: 25
    ## ── Column specification ────────────────────────────────────────────────────────
    ## Delimiter: ","
    ## chr (18): nppes_provider_last_org_name, nppes_provider_first_name, nppes_pro...
    ## dbl  (7): npi, line_srvc_cnt, bene_unique_cnt, bene_day_srvc_cnt, average_me...
    ## 
    ## ℹ Use `spec()` to retrieve the full column specification for this data.
    ## ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
...
if (!exists("dat.MDPPAS")) dat.MDPPAS <- vroom(here("Output", "dat.MDPPAS.csv"))
```
...
``` r
if (!exists("dat.PUF")) dat.PUF <- vroom(here("Output", "dat.PUF.csv"))
```
...
``` r
if (!exists("dat.PFS")) dat.PFS <- vroom(here("Output", "dat.PFS.csv"))
```
...
1.  [MD-PPAS](https://resdac.org/cms-data/files/md-ppas); The Medicare
    ...
    Review total physician-level Medicare spending, claims, and
    patients.
...
``` r
  dat.PUF %>% 
      select(Total_Spending='average_medicare_payment_amt', 
             Total_Claims='average_submitted_chrg_amt', 
             Total_Patients='line_srvc_cnt') %>%
...
```
    ...
    ##   <chr>    <dbl>    <dbl>     <dbl>    <dbl>
    ## 1 Spending  78.8     174. 0           31537.
    ## 2 Claims   331.      873. 0.0000333   99999 
    ## 3 Patients 199.     2273. 2.4       5313837

``` r
  dat.PUF %>% 
      group_by(npi) %>% 
      summarise_at("line_srvc_cnt", mean) %>% 
      ggplot(aes(x = npi, y = line_srvc_cnt, group=npi)) + 
      theme_minimal() + 
      geom_point()
```

![](Main_files/figure-gfm/q1-1.png)<!-- -->

1.  Form a proxy for integration using the ratio:

2.  where $HOPD_{it}$ reflects the total number of claims in which
    physician $i$ bills in a hospital outpatient setting, $OFFICE_{it}$
    is the total number of claims billed to an office setting, and
    $ASC_{it}$ is the total number of claims billed to an ambulatory
    surgery center. Plot the mean of total physician-level claims for
    integrated versus non-integrated physicians over time (just pick a
    few thresholds for defining “integration”).
