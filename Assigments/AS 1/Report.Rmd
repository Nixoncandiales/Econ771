...
dummy and a relative time dummy. In this notation and context, $\tau$
denotes years relative to Medicaid expansion, so that $\tau=-1$ denotes
the year before a state expanded Medicaid, $\tau=0$ denotes the year of
...
kable(table1 %>% relocate(starts_with("unc"), starts_with("hosp")), format = "latex")


kable(table2 %>% relocate(year), format = "latex")
```

```{r table-3, include=TRUE, echo=FALSE, warning=FALSE, comment=NA, results='asis'}
stargazer(mod.twfe,
